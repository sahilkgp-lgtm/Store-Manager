...
# Admin Security
# -------------------------------------------
ADMIN_EMAIL = os.getenv("ADMIN_EMAIL", "sahilchhimpa029@gmail.com")
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not user or user["email"].strip().lower() != ADMIN_EMAIL.strip().lower():
...
@app.get("/api/suppliers/<int:supplier_id>/past-products")
@login_and_subscription_required
def get_supplier_past_products(supplier_id):
    user_id = session.get("user_id")
    query = """
        SELECT DISTINCT ON (LOWER(bi.material_name))
            bi.material_name,
            bi.brand,
            bi.batch_no,
            bi.rate AS last_rate,
            bi.cgst_pct,
            bi.sgst_pct,
            COALESCE(inv.mrp, 0) AS mrp,
            inv.expiry_date
        FROM public.bill_items bi
        JOIN public.bills b ON bi.bill_id = b.id
        LEFT JOIN public.inventory inv ON bi.inventory_id = inv.id
        WHERE b.user_id = %s 
          AND b.entity_id = %s 
          AND b.bill_type = 'PURCHASE'
          AND b.status = 'POSTED'
        ORDER BY LOWER(bi.material_name), b.created_at DESC;
    """
    with db_connection() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(query, (user_id, supplier_id))
            items = cur.fetchall()
    ...
    query = """
        SELECT 
            ...
            CASE 
                ...
                ELSE COALESCE(s.agency_name, 'Unknown Supplier')
            END AS party_name
        FROM public.bills b
...
def get_single_bill(id):
    ...
    query = """
        SELECT 
            ...
            END AS party_name,
            CASE 
                WHEN b.bill_type = 'SALE' THEN c.phone
                ELSE s.phone
            END AS party_phone,
            CASE 
                WHEN b.bill_type = 'SALE' THEN c.location
                ELSE s.address
            END AS party_address,
            CASE 
                WHEN b.bill_type = 'SALE' THEN c.gstin
                ELSE s.gstin
            END AS party_gstin
        FROM public.bills b
        LEFT JOIN public.customers c ON b.entity_id = c.id AND b.bill_type = 'SALE'
...
def get_single_ledger_entry(id):
    ...
    query = """
        SELECT 
            ...
            l.created_at,
            CASE 
                WHEN l.entity_type = 'CUSTOMER' THEN COALESCE(c.name, 'Customer #' || l.entity_id)
                ELSE COALESCE(s.agency_name, 'Supplier #' || l.entity_id)
            END AS party_name,
            CASE 
                WHEN l.entity_type = 'CUSTOMER' THEN COALESCE(c.phone, '—')
                ELSE COALESCE(s.phone, '—')
            END AS party_phone,
            CASE 
                WHEN l.entity_type = 'CUSTOMER' THEN COALESCE(c.location, '—')
                ELSE COALESCE(s.address, '—')
            END AS party_address,
            CASE 
                WHEN l.entity_type = 'CUSTOMER' THEN COALESCE(c.gstin, '—')
                ELSE COALESCE(s.gstin, '—')
            END AS party_gstin,
            CASE 
                WHEN l.entity_type = 'CUSTOMER' THEN COALESCE(c.dues, 0)
                ELSE COALESCE(s.outstanding_balance, 0)
