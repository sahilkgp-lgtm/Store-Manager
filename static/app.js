document.addEventListener("DOMContentLoaded", () => {
  ...
  supplierPastProducts: [], // stores items bought from current supplier
  ...
  function resetBill(kind) {
    ...
    if (kind === "SALE") { 
      ...
      state.purchaseCart = []; 
      $("purchaseParty").value = ""; 
      if ($("purchaseSupplierSearch")) $("purchaseSupplierSearch").value = "";
    }
  ...
  async function saveBill(kind) {
    ...
    if (kind === "PURCHASE" && !partyId) {
      return toast("Please select a supplier from the dropdown suggestions.", true);
    }

    const payload = { 
      entity_id: partyId ? Number(partyId) : null, 
      items: cart, 
    ...
    try {
      ...
      // Auto-print invoice immediately after saving
      if (response.id) {
        printBill(response.id);
      }
  ...
  async function printBill(id) {
    ...
    try {
      ...
      const rows = (bill.items || []).map((item, idx) => {
        ...
        return `<tr>
          ...
          <td>
            ...
            ${item.brand ? `<span style="font-size: 11px;"> (${escapeHtml(item.brand)})</span>` : ""}
            <div style="font-size: 11px; color: #333;">Batch: ${escapeHtml(item.batch_no || '—')}</div>
          </td>
          <td style="text-align: right;">${number(item.qty).toFixed(2)}</td>
          <td style="text-align: right;">${money(item.rate)}</td>
          <td style="text-align: right;">${number(item.cgst_pct)}%<br><span style="font-size: 11px;">${money(cgstAmt)}</span></td>
          <td style="text-align: right;">${number(item.sgst_pct)}%<br><span style="font-size: 11px;">${money(sgstAmt)}</span></td>
      ...
      const partyTitle = isSale ? "BILLED TO (CUSTOMER):" : "PURCHASED FROM (SUPPLIER):";
      const balanceDue = number(bill.total_amount) - number(bill.paid_amount);
      ...
      popup.document.write(`
        ...
        <head>
          ...
          <style>
            ...
            body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif; color: #000; background: #fff; margin: 0; padding: 10px; font-size: 12px; line-height: 1.4; }
            .invoice-wrapper { border: 1.5px solid #000; padding: 18px 20px; min-height: 94vh; box-sizing: border-box; display: flex; flex-direction: column; justify-content: space-between; }
            .header { text-align: center; border-bottom: 2px solid #000; padding-bottom: 10px; margin-bottom: 12px; }
            .header h1 { margin: 0 0 4px 0; font-size: 22px; font-weight: 800; letter-spacing: 0.8px; text-transform: uppercase; }
            .header p { margin: 2px 0; font-size: 11px; }
            .title-badge { text-align: center; font-size: 12px; font-weight: 700; letter-spacing: 1px; border: 1.5px solid #000; padding: 5px; margin-bottom: 12px; text-transform: uppercase; }
            .info-grid { display: grid; grid-template-columns: 1.2fr 1fr; border: 1.5px solid #000; margin-bottom: 14px; }
            .info-col { padding: 8px 10px; }
            .info-col:first-child { border-right: 1.5px solid #000; }
            .info-col strong { font-size: 11px; text-transform: uppercase; }
            .party-name { font-size: 14px; font-weight: bold; margin-top: 3px; }
            table { width: 100%; border-collapse: collapse; margin-top: 4px; }
            th, td { border: 1.5px solid #000; padding: 6px 8px; vertical-align: middle; }
            th { background: #fff; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.3px; }
            .bottom-section { display: flex; justify-content: flex-end; margin-top: 14px; }
            .totals-table { width: 320px; border-collapse: collapse; }
            .totals-table td { border: 1.5px solid #000; padding: 5px 8px; font-size: 11px; }
            .totals-table td.label { font-weight: 600; }
            .totals-table td.val { text-align: right; font-variant-numeric: tabular-nums; }
            .totals-table tr.highlight td { font-size: 12px; font-weight: bold; }
            .sign-area { margin-top: 50px; display: flex; justify-content: space-between; padding: 0 20px; }
            .sign-box { width: 220px; border-top: 1.5px solid #000; text-align: center; padding-top: 6px; font-size: 11px; font-weight: 600; }
          </style>
        </head>
        <body>
          <div class="invoice-wrapper">
            <div>
              ...
              <div class="title-badge">${escapeHtml(bill.bill_type)} TAX INVOICE</div>

              <div class="info-grid">
                <div class="info-col">
                  <strong>${partyTitle}</strong>
                  <div class="party-name">${escapeHtml(bill.party_name || "Cash Sale")}</div>
                  ${bill.party_phone ? `<div>Phone: ${escapeHtml(bill.party_phone)}</div>` : ""}
                  ${bill.party_address ? `<div>Address: ${escapeHtml(bill.party_address)}</div>` : ""}
                  ${bill.party_gstin ? `<div>GSTIN: ${escapeHtml(bill.party_gstin)}</div>` : ""}
                </div>
                <div class="info-col">
                  <div><strong>INVOICE #:</strong> #${bill.id}</div>
                  <div><strong>DATE:</strong> ${new Date(bill.created_at).toLocaleString()}</div>
                  <div><strong>STATUS:</strong> ${bill.status === "POSTED" ? "Processed" : escapeHtml(bill.status)}</div>
                </div>
              </div>
              ...
              <div class="bottom-section">
                <table class="totals-table">
                  ...
                  <tr>
                    ...
                    <td class="val">${money(cgstTotal)}</td>
                  </tr>
                  <tr>
                    <td class="label">SGST Total:</td>
                    <td class="val">${money(sgstTotal)}</td>
                  </tr>
                  <tr>
                    <td class="label">Transport Charges:</td>
                    <td class="val">${money(bill.transport_charges)}</td>
                  </tr>
                  <tr class="highlight">
                    <td class="label">Grand Total:</td>
                    <td class="val">${money(bill.total_amount)}</td>
                  </tr>
                  <tr>
                    <td class="label">Amount Paid:</td>
                    <td class="val">${money(bill.paid_amount)}</td>
                  </tr>
                  <tr class="highlight">
                    <td class="label">Balance Due:</td>
                    <td class="val">${money(balanceDue)}</td>
                  </tr>
                </table>
              </div>
            </div>
            ...
            <div class="sign-area">
              <div class="sign-box">${isSale ? "Customer Signature" : "Supplier Signature"}</div>
  ...
  // --- UNIFIED PARTY STATEMENT PRINTING (CUSTOMERS & SUPPLIERS) ---
async function printStatement(selectedOnly = false) {
  ...
  try {
    ...
    const popup = window.open("", "_blank", "width=850,height=800");
    if (!popup) return toast("Allow pop-ups to print statement.", true);
    ...
    const tableRows = rowsToPrint.map((row, idx) => {
      ...
      const isBill = row.record_type === "BILL";
      ...
      // 1. Shows RECEIPT or PAYMENT instead of LEDGER
      const displayRecordType = isBill 
        ? "BILL" 
        : (row.status === "RECEIPT" ? "RECEIPT" : "PAYMENT");

      // 2. Status / Nature description
      const rowStatus = row.status === "POSTED" 
        ? "Processed" 
        : (row.status === "RECEIPT" ? "Receive/Receipt" : (row.status === "PAYMENT" ? "Sent/Paid" : row.status));

      return `<tr>
        <td style="text-align: center;">${idx + 1}</td>
        <td>${new Date(row.date).toLocaleString()}</td>
        <td><strong>${escapeHtml(displayRecordType)}</strong></td>
        <td>#${escapeHtml(row.ref_id || '-')}</td>
        <td style="text-align: center;">${escapeHtml(rowStatus)}</td>
        <td style="text-align: right; font-weight: bold;">${money(row.amount)}</td>
        <td style="text-align: right; font-weight: bold;">${money(paid)}</td>
      </tr>`;
    }).join("");

    const isCust = type === "CUSTOMER";
    const partyTitle = isCust ? "BILLED TO (CUSTOMER):" : "PURCHASED FROM (SUPPLIER):";
    const partyName = escapeHtml(data.party.name || "—");
    const partyPhone = escapeHtml(data.party.phone || "—");
    const partyLocation = escapeHtml(data.party.address || "—");
    const partyGstin = escapeHtml(data.party.gstin || "—");

    // Hide summary totals when printing selected rows only
    const summarySectionHTML = selectedOnly ? "" : `
      <div class="bottom-section">
        <table class="totals-table">
          <tr>
            <td class="label">Total Invoiced:</td>
            <td class="val">${money(totalInvoiced)}</td>
          </tr>
          <tr>
            <td class="label">Total Payments / Receipts:</td>
            <td class="val">${money(totalPaid)}</td>
          </tr>
          <tr class="highlight">
            <td class="label">Net Balance Due:</td>
            <td class="val">${money(data.party.pending)}</td>
          </tr>
        </table>
      </div>
    `;
    ...
    popup.document.write(`
      ...
      <head>
        <meta charset="utf-8">
        <title>${type} STATEMENT - ${partyName}</title>
        <style>
          @page { size: A4 portrait; margin: 10mm 12mm; }
          body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif; color: #000; background: #fff; margin: 0; padding: 10px; font-size: 12px; line-height: 1.4; }
          .invoice-wrapper { border: 1.5px solid #000; padding: 18px 20px; min-height: 94vh; box-sizing: border-box; display: flex; flex-direction: column; justify-content: space-between; }
      ...
      <body>
        <div class="invoice-wrapper">
          <div>
            ...
            <div class="title-badge">${escapeHtml(type)} ACCOUNT STATEMENT</div>

            <div class="info-grid">
              <div class="info-col">
                <strong>${partyTitle}</strong>
                <div class="party-name">${partyName}</div>
                <div>Phone: ${partyPhone}</div>
                <div>Address: ${partyLocation}</div>
                <div>GSTIN: ${partyGstin}</div>
              </div>
              <div class="info-col">
                <div><strong>STATEMENT FOR:</strong> ${partyName}</div>
                <div><strong>PRINTED ON:</strong> ${new Date().toLocaleString()}</div>
                <div><strong>STATUS:</strong> ${selectedOnly ? "Selected Records" : "Processed"}</div>
              </div>
            </div>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th style="width: 90px;">RECORD TYPE</th>
                  <th style="width: 80px;">REF #</th>
                  <th style="width: 100px; text-align: center;">STATUS / NATURE</th>
          ...
          <div class="sign-area">
            <div class="sign-box">${isCust ? "Customer Signature" : "Supplier Signature"}</div>
            <div class="sign-box">Authorized Signatory</div>
          </div>
        </div>
      </body>
  ...
  async function printReceipt(id) {
    if (!id) return toast("Invalid voucher ID.", true);
    try {
      ...
      const isReceipt = String(entry.entry_type).toUpperCase() === "RECEIPT";
      const docTitle = isReceipt ? "RECEIVE / RECEIPT VOUCHER" : "SENT / PAID VOUCHER";
      const partyTitle = isReceipt ? "BILLED TO (CUSTOMER):" : "PURCHASED FROM (SUPPLIER):";
      ...
      popup.document.write(`
        ...
        <head>
          ...
          <style>
            ...
            .party-name { font-size: 14px; font-weight: bold; margin-top: 3px; }
            table { width: 100%; border-collapse: collapse; margin-top: 4px; }
            th, td { border: 1.5px solid #000; padding: 8px 10px; vertical-align: middle; }
            th { background: #fff; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.3px; }
            .bottom-section { display: flex; justify-content: flex-end; margin-top: 14px; }
            .totals-table { width: 320px; border-collapse: collapse; }
            .totals-table td { border: 1.5px solid #000; padding: 6px 8px; font-size: 11px; }
            .totals-table td.label { font-weight: 600; }
            .totals-table td.val { text-align: right; font-variant-numeric: tabular-nums; }
            .totals-table tr.highlight td { font-size: 12px; font-weight: bold; }
            .sign-area { margin-top: 50px; display: flex; justify-content: space-between; padding: 0 20px; }
        ...
        <body>
          <div class="invoice-wrapper">
            <div>
              ...
              <div class="title-badge">${docTitle}</div>

              <div class="info-grid">
                <div class="info-col">
                  <strong>${partyTitle}</strong>
                  <div class="party-name">${escapeHtml(entry.party_name || "—")}</div>
                  <div>Phone: ${escapeHtml(entry.party_phone || "—")}</div>
                  <div>Address: ${escapeHtml(entry.party_address || "—")}</div>
                  <div>GSTIN: ${escapeHtml(entry.party_gstin || "—")}</div>
                </div>
                <div class="info-col">
                  <div><strong>VOUCHER #:</strong> #${entry.id}</div>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th>DESCRIPTION OF SETTLEMENT</th>
                    <th style="width: 140px; text-align: center;">PAYMENT MODE</th>
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      <strong>${isReceipt ? "Credit Account Clearance / Customer Receipt" : "Supplier Balance Payment / Outflow"}</strong>
                      <div style="font-size: 11px; margin-top: 2px;">Settlement Entry Ref #${entry.id}</div>
                    </td>
                    <td style="text-align: center; font-weight: 600;">${escapeHtml(entry.payment_mode || "Cash")}</td>
                    <td style="text-align: right; font-weight: bold;">${money(entry.amount)}</td>
                  </tr>
                </tbody>
              </table>
              ...
              <div class="bottom-section">
                <table class="totals-table">
                  <tr class="highlight">
                    ...
                    <td class="val">${money(entry.amount)}</td>
                  </tr>
                  <tr>
                    <td class="label">Payment Mode:</td>
                    <td class="val">${escapeHtml(entry.payment_mode || "Cash")}</td>
                  </tr>
                  <tr class="highlight">
                    <td class="label">Remaining Balance Due:</td>
                    <td class="val">${money(entry.current_dues)}</td>
                  </tr>
                </table>
              </div>
            </div>
            ...
            <div class="sign-area">
              <div class="sign-box">${isReceipt ? "Customer Signature" : "Supplier Signature"}</div>
              <div class="sign-box">Authorized Signatory</div>
            </div>
          </div>
        </body>
  ...
  async function loadLedger() {
    try {
      ...
      $("ledgerTable").innerHTML = entries.length ? entries.map((entry) => {
        const typeDisplay = entry.entry_type === "RECEIPT" ? "Receive/Receipt" : "Sent/Paid";
        return `<tr>
          ...
          <td>
            <button type="button" class="table-button" data-action="print-receipt" data-id="${entry.id}">Print</button>
          </td>
        </tr>`;
      }).join("") : emptyRow(7, "No ledger entries recorded.");
    } catch (err) { toast(err.message, true); }
  }
  ...
  function renderBillHistoryTable() {
    ...
    $("historyTable").innerHTML = filtered.length ? filtered.map((txn) => {
      ...
      const rawStatus = txn.status || txn.bill_type || "POSTED";
      const displayStatus = rawStatus === "POSTED" ? "Processed" : (rawStatus === "RECEIPT" ? "Receive/Receipt" : (rawStatus === "PAYMENT" ? "Sent/Paid" : rawStatus));
      
      let actionButtons = "";
      if (isLedger) {
        actionButtons = `<button class="table-button" data-action="print-receipt" data-id="${cleanId}">Print</button>`;
      } else if (rawStatus === "POSTED") {
  ...
  async function showStatement(type, id) {
    try {
      ...
      $("statementTable").innerHTML = data.history.length ? data.history.map(row => {
        const rowStatus = row.status === "POSTED" ? "Processed" : (row.status === "RECEIPT" ? "Receive/Receipt" : (row.status === "PAYMENT" ? "Sent/Paid" : row.status));
        return `
        <tr>
  ...
  if (event.target.id === "purchaseName") {
      const term = event.target.value.toLowerCase().trim();
      const listDiv = $("purchaseProductDropdown");
      if (!term || !state.supplierPastProducts || !state.supplierPastProducts.length) {
        listDiv?.classList.add("hidden");
        return;
      }

      const filtered = state.supplierPastProducts.filter(p => 
      ...
      listDiv.innerHTML = filtered.map(p => {
        // Find the actual index inside the global state array
        const realIndex = state.supplierPastProducts.indexOf(p);
        return `
          <div class="purchase-product-item" data-index="${realIndex}" style="padding: 8px 12px; cursor: pointer; border-bottom: 1px solid #e2e8f0; color: #1e293b; font-size: 13px;">
            <strong>${escapeHtml(p.material_name)}</strong> ${p.brand ? `(${escapeHtml(p.brand)})` : ""}<br>
            <small style="color: #64748b;">Last Rate: ${money(p.last_rate)} | MRP: ${money(p.mrp)} | Batch: ${escapeHtml(p.batch_no || "—")}</small>
          </div>
        `;
      }).join("");
      listDiv.classList.remove("hidden");
  ...
  const pastProductItem = event.target.closest(".purchase-product-item");
    if (pastProductItem) {
      const idx = Number(pastProductItem.dataset.index);
      const item = state.supplierPastProducts[idx];
      
      if (item) {
        $("purchaseName").value = item.material_name || "";
        $("purchaseBrand").value = item.brand || "";
        $("purchaseBatch").value = item.batch_no || "";
        $("purchaseRate").value = item.last_rate != null ? Number(item.last_rate) : "";
        $("purchaseMrp").value = item.mrp != null ? Number(item.mrp) : "";
        $("purchaseCgst").value = item.cgst_pct != null ? Number(item.cgst_pct) : 0;
        $("purchaseSgst").value = item.sgst_pct != null ? Number(item.sgst_pct) : 0;
        
        if (item.expiry_date) {
          $("purchaseExpiry").value = String(item.expiry_date).slice(0, 10);
        } else {
          $("purchaseExpiry").value = "";
        }
      }
      
      $("purchaseProductDropdown")?.classList.add("hidden");
      return;
    }
    if (!event.target.closest("#purchaseName") && !event.target.closest("#purchaseProductDropdown")) {
      $("purchaseProductDropdown")?.classList.add("hidden");
    }

    // Supplier Search in Purchase Bill
    if (event.target.id === "purchaseSupplierSearch") {
      const term = event.target.value.toLowerCase();
      const listDiv = $("supplierDropdownList");
      if (!term) { listDiv.classList.add("hidden"); $("purchaseParty").value = ""; return; }
      const filtered = state.suppliers.filter(s => (s.agency_name || "").toLowerCase().includes(term) || String(s.phone || "").includes(term));
      if (!filtered.length) { 
        listDiv.innerHTML = `<div style="padding: 8px; color: #64748b;">No suppliers found</div>`; 
      } else { 
        listDiv.innerHTML = filtered.map(s => `
          <div class="supplier-suggestion-item" data-id="${s.id}" data-name="${escapeHtml(s.agency_name)} · Due ${money(s.outstanding_balance)}" style="padding: 8px 12px; cursor: pointer; border-bottom: 1px solid #e2e8f0; color: #1e293b;">
            <strong>${escapeHtml(s.agency_name)}</strong> · Due ${money(s.outstanding_balance)}
          </div>
        `).join(""); 
      }
      listDiv.classList.remove("hidden");
  ...
  document.addEventListener("click", async (event) => {
    // 1. Customer click selection
    const customerItem = event.target.closest(".customer-suggestion-item");
    if (customerItem) {
      $("saleParty").value = customerItem.dataset.id; 
      $("saleCustomerSearch").value = customerItem.dataset.name;
    ...
    // 2. Supplier click selection
    // Supplier click selection
const supplierItem = event.target.closest(".supplier-suggestion-item");
if (supplierItem) {
  $("purchaseParty").value = supplierItem.dataset.id; 
  $("purchaseSupplierSearch").value = supplierItem.dataset.name;
  $("supplierDropdownList")?.classList.add("hidden");

  // Fetch past products purchased from this supplier
  api(`/api/suppliers/${supplierItem.dataset.id}/past-products`).then(products => {
    state.supplierPastProducts = products || [];
  }).catch(() => {
    state.supplierPastProducts = [];
  });
  return;
}

    if (!event.target.closest("#purchaseSupplierSearch") && !event.target.closest("#supplierDropdownList")) { 
      $("supplierDropdownList")?.classList.add("hidden"); 
    }

    // Allow button OR any element with data-action (e.g. <a data-action="print-receipt">)
    const button = event.target.closest("button") || event.target.closest("[data-action]"); 
    if (!button) return;
